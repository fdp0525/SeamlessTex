    ...
    Settings const & settings)
{
    ...
    for (std::size_t i = 0; i < faces.size(); ++i)
    {
        for (std::size_t j = 0; j < 3; ++j)
        {
            ...
            math::Vec2f pixel = texture_view.get_pixel_coords_noshift(vertex);
...
//    assert(max_y < view_image->height());
    ...
    if(max_x > view_image->width() -1 )
    {
        max_x = view_image->width() - 1;
    }
    if(max_y > view_image->height() - 1)
    {
        max_y = view_image->height() - 1;
    }
...
generate_texture_patches(UniGraph const & graph, mve::TriangleMesh::ConstPtr mesh,
    ...
    std::vector<std::vector<VertexProjectionInfo> > * vertex_projection_infos,
    std::vector<TexturePatch::Ptr> * texture_patches, int passiter)
{
...
//        texture_view->release_image();
...

    {
        ...
        if(passiter == 2)
        {
            graph.get_twoPassSubgraphs(0, &subgraphs);
