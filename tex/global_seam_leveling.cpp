...
sample_edge(TexturePatch::ConstPtr texture_patch, math::Vec2f p1, math::Vec2f p2) {
    ...
    for (std::size_t s = 0; s < num_samples; ++s) {
        ...
        if(sample_point(0) < 0 || sample_point(0) > G2LTexConfig::get().IMAGE_WIDTH - 1 ||
                sample_point(1) < 0 || sample_point(1) > G2LTexConfig::get().IMAGE_HEIGHT - 1)
        {
            continue;
        }
...
//    assert(!std::isnan(difference[2]));
    ...
    if(std::isnan(difference[0]) || std::isnan(difference[1]) || std::isnan(difference[2]))
    {
        difference[0] = difference[1] = difference[2] = 0.0f;
    }
