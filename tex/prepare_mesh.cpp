...
{
    std::size_t num_redundant = remove_redundant_faces(*mesh_info, mesh);//去掉重复的三角网格，其实这一步与处理已经处理了。
    if (num_redundant > 0)
    {
    ...
    mesh->ensure_normals(true, true);//计算网格的面法线和顶点法线

    /* Update vertex infos. */
    mesh_info->clear();
    mesh_info->initialize(mesh);//计算每个顶点，并给出每个顶点的类型，1-ring邻接面以及顶点
}
