...
/* ---------------------------------------------------------------- */
/**
 * @brief TriangleMesh::ensure_normals 计算网格的面法线和顶点法线
 * @param face
 * @param vertex
 */
...
{
    //判断是否重新计算
    bool need_face_recalc = (face && this->face_normals.size() != this->faces.size() / 3);
    bool need_vertex_recalc = (vertex && this->vertex_normals.size() != this->vertices.size());

    this->recalc_normals(need_face_recalc, need_vertex_recalc);//计算网格的面法线和顶点法线
}
